...
function readBoolOrStringValue(val: any): string | boolean {
    const val1 =  val as boolean;
...
const CreateActivity = ({ options }: ICreateActivityProps) => {
    ...
    const ParameterPropertyInput = ({ index, parameter, field }: { index: number, parameter: IActivityParam, field: keyof IActivityParam }) => (
        <VSCodeTextField value={parameter[field] as string || ''} onChange={ev => updateParameter(index, { ...parameter, [field]: (ev.target as any).value })}></VSCodeTextField>
    );
